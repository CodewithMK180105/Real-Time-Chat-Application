...
const Dashboard = () => {
  ...
  const fetchMessages = async (conversationId, user) => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/message/${conversationId}?userId=${user.id}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      setMessages({messages: resData, receiver: user}); // Directly set the response array
    } catch (error) {
        ...
        <div className="mx-5 mt-5">
          ...
          <div>
            {conversations.length > 0 ? (
              conversations.map((conversation) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => fetchMessages(conversation.conversationId,user)}
                  >
                    <div
              ...
              <div className="text-center text-lg font-semibold mt-12">No Messages</div>
            )}
          </div>
        </div>
      </div>
          ...
          <div className="p-10">
            {
              ...
              messages.messages.map(({message, user: {id}={}}) => {
                return(
                  <div key={message} className={`max-w-[40%] rounded-b-xl p-4 mb-6 ${id===user.id?'bg-primary rounded-tl-xl ml-auto text-white':'bg-secondary rounded-tr-xl'}`}>
                    {message}
                  </div>
                )
